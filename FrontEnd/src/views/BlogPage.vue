...
export default {
  ...
  mounted() {
    this.$store.state.location.isMyblog = true;
    axios({
        methos: "GET",
        url: this.$baseurl+'/user/bloguser/' + this.$route.params.uid,
         headers: {
          Authorization: this.$store.state.userm.token
        }
        // url: 'http://localhost:8082/blog/user/bloguser/' + this.$route.params.uid
      })
        .then((resp) => {
